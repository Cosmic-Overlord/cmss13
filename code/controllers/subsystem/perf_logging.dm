...
/datum/controller/subsystem/perf_logging/proc/update_mc_timing()
	...
	timing_info.client_count = length_char(GLOB.clients)
	timing_info.human_count = length_char(SShuman.processable_human_list)
	timing_info.xeno_count = length_char(GLOB.xeno_mob_list)
