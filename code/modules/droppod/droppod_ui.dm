...
/datum/admin_podlauncher/proc/pre_launch() //Creates a list of acceptable items,
	...
	if (length_char(acceptableTurfs) && !custom_dropoff) //We dont fill the supplypod if acceptableTurfs is empty, if the pod is going in reverse (effectReverse=true), or if the pod is acitng like a missile (effectMissile=true)
