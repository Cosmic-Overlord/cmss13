...
/proc/return_generator_args(generator/target)
	...
	string_repr = copytext_char(string_repr, 11, length_char(string_repr)) // strips extraneous data
	string_repr = replacetext_char(string_repr, "\"", "") // removes the " around the type
	return splittext_char(string_repr, ", ")
